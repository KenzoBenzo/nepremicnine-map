...
import { HTMLMotionProps, MotionProps, useMotionValue, useTransform } from 'framer-motion'

const PanelContainer = ({ children, min, side }: { children: ReactNode, min: number, side?: "left" | "right" } & BoxProps & MotionProps) => {
  return (
    <MotionBox
      position='fixed'
      top={0}
      sx={{ left: side === 'left' ? 0 : null, right: side === 'right' ? 0 : null }}
      width={`${min}px`}
      h="100%"
      minH="100vh"
...
const DragHandle = ({ min, max, side }: { min: number, max: number, side?: "left" | "right" } & MotionProps & BoxProps) => {
  return (
    <MotionBox
      position='fixed'
      top={0}
...
const MapPanel = ({ children, side = "right", min = 280, max = 600 }: MapPanelProps) => {
